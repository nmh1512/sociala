    ...
    <div class="min-h-screen bg-light-theme-body relative">
                ...
                <div class="flex h-full items-center justify-between">
                    <div class="flex items-center gap-x-2 md:gap-x-4  xl:gap-x-20">
                        <div class="flex-shrink-0">
                            <router-link :to="{ name: 'Home' }"
                                class="flex items-center gap-x-2.5 text-color-theme-blue">
                            ...
                            <div class="">
                                <form action="">
                                    <div class="relative flex items-center text-gray-400 focus-within:text-gray-600">
                                        <MagnifyingGlassIcon class="w-4 h-4 absolute ml-2 md:ml-3 pointer-event-none" />
                            ...
                            <div class="ml-3 lg:ml-10 flex items-baseline space-x-2 lg:space-x-4">
                                <router-link v-for="item in navigation" :key="item.name" :to="item.to" :class="[
                                    ...
                                    <component :is="item.icon"
                                        class="inline-block w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 p-2 lg:p-3" />
                                </router-link>
                            </div>
                        </div>
                    </div>
                    <div class="hidden sm:block">
                        <div class="flex items-center lg:ml-6 gap-x-2 md:gap-x-6">
                            <button type="button" class="relative text-color-theme-blue">
                                <span class="absolute -inset-1.5" />
                                <BellIcon class="h-6 w-6 md:h-8 md:w-8" aria-hidden="true" />
                            </button>
                            <button type="button" class="relative text-color-theme-blue" @click="handleToogleNavChat">
                                <span class="absolute -inset-1.5" />
                            ...
                            <Menu as="div" class="relative">
                                <div>
                                    <MenuButton
                                        ...
                                        <img class="h-6 w-6 md:h-8 md:w-8 rounded-full" :src="user.imageUrl" alt="" />
                                    </MenuButton>
                                </div>
                    ...
                    <div class="-mr-2 flex sm:hidden gap-x-4">
                        ...
                        <!-- Mobile menu button -->
                        <DisclosureButton class="relative inline-flex items-center justify-center"
                            @click="handleToogleNavLeft">
                            <span class="absolute -inset-0.5" />
                            <span class="sr-only">Open main menu</span>
                            <Bars3BottomRightIcon v-if="!open" class="block h-6 w-6 text-gray-900" aria-hidden="true"
                                @click="handleToogleNavLeft" />
                            <XMarkIcon v-else class="block h-6 w-6 text-gray-900" aria-hidden="true" />
                        </DisclosureButton>
                    </div>
                </div>
            </div>
        ...
        <div :class="[
            ...
            <form class="w-full" action="">
                <input type="text" name="search" :placeholder="$t('search_placeholder_mobile')" autocomplete="off"
                    :aria-label="$t('search_placeholder_mobile')"
                    class="pr-3 pl-5 px-5 py-4 border-none w-full focus:border-none focus:ring-0" />
            </form>
            <div class="flex items-center">
                <XMarkIcon class="block h-7 w-7 mr-2 text-gray-400" aria-hidden="true"
